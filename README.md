...
- 宣言している権限は `activeTab` と `scripting` の 2 つのみで、`<all_urls>` のような広範な host permissions は要求しません。コンテンツスクリプトとスタイルシートはユーザーがショートカットを押したり、ツールバーアイコンをクリックしたタイミングで、現在アクティブなタブにのみ `chrome.scripting` で注入されます。
...
1. ユーザーがショートカット／ツールバーアイコンでトグルすると、Service Worker がまず既存のコンテンツスクリプトに `TOGGLE` メッセージを送ります。受信側がいなければ（初回または別ページ遷移後）、`activeTab` 権限の下で `chrome.scripting.insertCSS` / `chrome.scripting.executeScript` により当該タブにのみ `styles.css` と `content.js` を注入し、改めて `TOGGLE` を送ります。
2. コンテンツスクリプトは `en` → `ja` の `Translator` インスタンスがあることを確認します。モデル未ダウンロードの場合は、ユーザー操作によるダウンロード開始を促すバナーを表示します。
3. `document.body` をブロック単位（`<p>`, `<li>`, `<div>` など）で走査し、`<script>`, `<style>`, `<code>`, `<pre>`、contenteditable 領域、すでに注入済みのノードはスキップします。
4. 各ブロック内では、隣接するテキストノードとインライン要素（`<a>`, `<em>` など）のテキストを連結したフラットなストリームを `Intl.Segmenter` で文に分割します。UI ラベルらしき文（ボタン内の短いテキスト、単独で短いリンク、ラベル類）や、リンクテキストが URL そのものの文は除外します。
5. 残った文の直後に `<span class="subtitler-loading">翻訳中...</span>` のプレースホルダを挿入します。
6. 各プレースホルダを `IntersectionObserver` で監視し、ビューポートに入った（200px のマージン付き）時点で翻訳キューに投入します。
7. 同時実行数の上限を 4 とした小さなキューが処理を行い、翻訳結果が返るとプレースホルダを `<span class="subtitler-ja">…</span>` に置き換えます。
8. `MutationObserver` により後から追加された DOM ノード（SPA のページ遷移、遅延読み込みされたセクションなど）を捕捉し、同じパイプラインで処理します。自身が注入したノードは `WeakSet` で記録してフィードバックループを防ぎます。
9. 表示のオン／オフ切り替えは、注入済みの全要素のインライン `display` を反転するだけで済み、再翻訳は行いません。

## ファイル構成

```
extension/
  manifest.json   # MV3 マニフェスト、commands、activeTab + scripting
